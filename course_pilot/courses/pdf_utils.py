...
def generate_courses_pdf(eligible_programmes, user_points, user_grades):
    ...
    # User information
    user_info = Paragraph(f"<b>Your Cluster Points:</b> {user_points}", normal_style)
    story.append(user_info)
    story.append(Spacer(1, 0.1*inch))
    
    # Eligible courses section
    courses_header = Paragraph("Eligible Degree Programs", header_style)
    story.append(courses_header)
    story.append(Spacer(1, 0.1*inch))
    
    if not eligible_programmes:
        no_courses = Paragraph("No eligible courses found matching your criteria.", normal_style)
